...
namespace CommandAS.Tools.Forms
{
	...
	/// 
	/// </summary>
	public class dlgSelectTV2 : System.Windows.Forms.Form
	{
		#region Visual vars
		...
		{
			mTV.DoubleClick	+= new System.EventHandler(DoCommand);
			if (pIsLoadByObj)
			{
				if (mPCLoadingItem.code > 0)
					mTV.Load(mPCLoadingItem, false); 
				else
					mTV.Load();
		...
		{
			if ((mTV.SelectedNode != null && mTV.SelectedNode.Nodes.Count == 0) || mTV.pIsMaySelectedNode)
			{
				mSelectedNode = mTV.SelectedNode; 
