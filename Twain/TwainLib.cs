...
namespace CommandAS.Tools.TwainLib
{
	...
	public class Twain
	{
    private TwUserInterface guif = new TwUserInterface();
	...
	public TwainCommand PassMessage( ref Message m )
		...
		int pos = User32_DLL.GetMessagePos();

		winmsg.hwnd		= m.HWnd;
		winmsg.message	= m.Msg;
		winmsg.wParam	= m.WParam;
		winmsg.lParam	= m.LParam;
		winmsg.time		= User32_DLL.GetMessageTime();
		winmsg.x		= (short) pos;
  ...
  //  }
    ...
    public void CloseSrc()
    {
      TwRC rc;
      if (srcds.Id != IntPtr.Zero)
      //if ((srcds.Id != IntPtr.Zero) & (isDataSourceOpen))
      {
        if (guif.ParentHand != IntPtr.Zero)
        {
          rc = DSuserif(appid, srcds, TwDG.Control, TwDAT.UserInterface, TwMSG.DisableDS, guif);
	...
	public static int ScreenBitDepth {
		get {
			IntPtr screenDC = GDI32_DLL.CreateDC( "DISPLAY", null, null, IntPtr.Zero );
			int bitDepth = GDI32_DLL.GetDeviceCaps( screenDC, 12 );
			bitDepth *= GDI32_DLL.GetDeviceCaps( screenDC, 14 );
			GDI32_DLL.DeleteDC( screenDC );
			return bitDepth;
			}
		}
